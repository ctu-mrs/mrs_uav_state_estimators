...
private:
  ...
  std::unique_ptr<GpsGarmin>    gps_garmin_estimator_;
...
void EstimationTesting::onInit() {
  ...
  gps_garmin_estimator_ = std::make_unique<GpsGarmin>();

  gps_garmin_estimator_->initialize(nh_);

  for (int i = 0; i < 10; i++) {
    if (gps_garmin_estimator_->isReady()) {
      ROS_INFO("[OdometryTesting]: starting GPS_GARMIN estimator");
      gps_garmin_estimator_->start();
      break;
    }
    ROS_INFO("[OdometryTesting]: waiting for GPS_GARMIN estimator to be ready");
