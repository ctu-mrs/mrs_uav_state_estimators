...
void GpsBaro::initialize(ros::NodeHandle &nh, const std::shared_ptr<CommonHandlers_t> &ch) {
  ...
  timer_update_             = nh.createTimer(ros::Rate(ch_->desired_uav_state_rate), &GpsBaro::timerUpdate, this, false, false);  // not running after init
  timer_check_health_       = nh.createTimer(ros::Rate(ch_->desired_uav_state_rate), &GpsBaro::timerCheckHealth, this);
  timer_pub_attitude_       = nh.createTimer(ros::Rate(ch_->desired_uav_state_rate), &GpsBaro::timerPubAttitude, this);
...
/*//{ getInnovation() */
nav_msgs::Odometry GpsBaro::getInnovation() const {
  std::scoped_lock lock(mtx_innovation_);
  return innovation_;
}
/*//}*/
