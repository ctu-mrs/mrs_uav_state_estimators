...
#include "estimators/state/passthrough.h"
...
/* initialize() //{*/
void Passthrough::initialize(ros::NodeHandle &nh, const std::shared_ptr<CommonHandlers_t> &ch) {
...
void Passthrough::timerCheckHealth(const ros::TimerEvent &event) {
  ...
  if (isInState(INITIALIZED_STATE)) {
    ...
    if (sh_passthrough_odom_.hasMsg()) {
      ...
      ROS_INFO_THROTTLE(1.0, "[%s]: Waiting for msg on topic %s", getPrintName().c_str(), sh_passthrough_odom_.topicName().c_str());
      return;
    }
  }
...
/*//{ getUavState() */
mrs_msgs::UavState Passthrough::getUavState() const {
  std::scoped_lock lock(mtx_uav_state_);
  return uav_state_;
}
/*//}*/
...
/*//{ getInnovation() */
nav_msgs::Odometry Passthrough::getInnovation() const {
  std::scoped_lock lock(mtx_innovation_);
  return innovation_;
}
/*//}*/
...
/*//{ getPoseCovariance() */
std::vector<double> Passthrough::getPoseCovariance() const {
  std::scoped_lock lock(mtx_covariance_);
...
/*//{ getTwistCovariance() */
std::vector<double> Passthrough::getTwistCovariance() const {
  std::scoped_lock lock(mtx_covariance_);
